...
format long
data = csvread('Tek009_ALL.csv',1); % Read the data
x = 290;
y = 1200;
t1 = data(x:end,2);
for i=2:(numel(t1))
...
t1(1) = 0;
t1 =t1./1e12;
CH1=smooth(data(x:end,10)/1e6);  % Vpwm
CH2=smooth(data(x:end,12)/1e6);  % Vgs,L
CH3=smooth(data(x:end,14)/1e6);  % Vgg,H
CH4=smooth(data(x:end,4)/1e6);  % Vsw
CH5=smooth(data(x:end,6)/1e6);  % V1H
CH6=smooth(data(x:end,8)/1e6);  % V1L
...
figure();
plot(t1*1e9,CH4+0.9,'Color' , yellow_color,'LineWidth',2.5);
set(gca,'FontSize',13)
xlim([0 100]);
ylim([-0.8 6.5]);
