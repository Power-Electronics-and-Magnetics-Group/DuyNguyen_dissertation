...
if (0)
    ...
    B3 = (min_iL_boost-dc_iL_boost);
    B4 = ((max_iL_boost-dc_iL_boost)/ exp(-alpha*(1-D)*Tsw)-B3*cos(wd*(1-D)*Tsw))/sin(wd*(1-D)*Tsw);
end
...
for k=1:(numel(t)-1)
    ...
    elseif (mod(t(k),Tsw) < Tsw)
        %iL_aef=[iL_aef (exp(-alpha*mod(t(k),Tsw))*(-B3*cos(wd*mod(t(k),Tsw)) - B4*sin(wd*mod(t(k),Tsw))))];
