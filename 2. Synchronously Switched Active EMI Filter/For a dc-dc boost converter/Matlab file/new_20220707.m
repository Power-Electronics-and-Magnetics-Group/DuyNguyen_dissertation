...
Ro_i = [0.235855 0.3144737 0.47171 00.943421 2.358553]; % Parasitic resistor of boost circuit
...
figure();
plot(n_i,abs(attenuation_mx(:,1)),'Color' , blue_color,'LineWidth',2.8);
hold on
plot(n_i,abs(attenuation_mx(:,2)),'Color' , yellow_color,'LineWidth',2.8);
plot(n_i,abs(attenuation_mx(:,3)),'Color' , red_color,'LineWidth',2.8);
plot(n_i,abs(attenuation_mx(:,4)),'Color' , black_color,'LineWidth',2.8);
plot(n_i,abs(attenuation_mx(:,5)),'Color' , magenta_color,'LineWidth',2.8);
xlim([0 1.5]);
ylim([20 75]);
set(gca,'FontSize',16)
label_x=xlabel('$n_{\tau}$','Interpreter','latex','FontSize',28,'HorizontalAlignment','center');
...
legend('\it{Q_o}=1000','\it{Q_o}=750','\it{Q_o}=500','\it{Q_o}=250','\it{Q_o}=100','Location','southeast','FontSize',20);
legend('boxoff')
%%
h=gcf;
